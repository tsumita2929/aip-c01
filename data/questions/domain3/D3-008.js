...
window.DOMAIN3_QUESTIONS.push({
  ...
  "optionExplanations": {
    ...
    "B": {
      "correct": false,
      "text": "不正解です。Comprehend の毒性検出・PII 検出 API はコンテンツの安全性評価が可能であるものの、2つの API を個別に呼び出す必要があり、拒否トピックの柔軟な定義やカテゴリ別しきい値設定ができないため、統合管理の容易さという要件を満たしません。"
    },
    "C": {
      "correct": true,
    ...
    "D": {
      "correct": false,
      "text": "不正解です。バッチ推論は大量データの一括処理には適しているものの、リアルタイム処理ではなく不適切メッセージが表示された後の事後削除となるため、リアルタイムブロックという要件を満たしません。"
    }
  },
  "references": [
    {

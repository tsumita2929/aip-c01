...
window.DOMAIN3_QUESTIONS.push({
  ...
  "optionExplanations": {
    "A": {
      "correct": false,
      "text": "不正解です。Comprehend PII 検出は入力・出力それぞれに個別 API 呼び出しと Lambda 統合が必要であるものの、カスタム識別子の検出も追加実装が必要となり、運用効率が高いという要件を満たしません。"
    },
    "B": {
      "correct": false,
      "text": "不正解です。ガードレールと Comprehend の2つのサービスの管理が必要であるものの、ガードレール単体のカスタム正規表現で対応できる保険証番号検出を別サービスで実装するのは運用効率が低下し、最も運用効率が高いという要件を満たしません。"
    },
    "C": {
      "correct": true,

...
window.DOMAIN2_QUESTIONS.push({
  ...
  "type": "multiple",
  "difficulty": "hard",
  "question": "企業が複雑なビジネスプロセスを自動化するマルチエージェントアプリケーションを設計しています。Amazon Bedrock のマルチエージェントコラボレーション機能を活用したエージェント設計パターンとして正しいものを2つ選んでください。",
  "options": [
    {
      "id": "A",
  ...
  "explanation": "正解は A と C です。スーパーバイザーパターン（A が正解）はオーケストレーターエージェントが全体を管理し、各専門エージェントにタスクを委任して結果を統合するパターンで、Bedrock のマルチエージェントコラボレーションが提供するコア機能です。パイプラインパターン（C が正解）は複数のエージェントが直列に処理を行い、各エージェントの出力が次のエージェントの入力となるパターンです。B は単一プロセスでのメモリ共有は推奨されるパターンではありません。D は複数エージェントの同時実行は可能です。E はデータベース経由は必須ではありません。（スキル2.5.5）",
  "optionExplanations": {
    "A": {
      "correct": true,
      "text": "正解です。スーパーバイザーパターンはオーケストレーターエージェントによるタスク委任と結果統合の要件を直接満たします。Bedrock のマルチエージェントコラボレーション機能として最小の実装負荷で実現できます。"
    },
    "B": {
      "correct": false,

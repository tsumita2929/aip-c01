...
window.DOMAIN2_QUESTIONS.push({
  ...
  "options": [
    {
      "id": "A",
      "text": "エージェントのアクショングループの Lambda 関数内にサーキットブレーカーパターンを実装し、連続エラー閾値超過時にオープン状態に遷移してキャッシュ済みの代替応答を返す。一定期間後にハーフオープン状態で試験的にAPIを呼び出し、成功すればクローズ状態に復帰する"
    },
    {
      "id": "B",
  ...
  "explanation": "正解は A です。Lambda 関数内にサーキットブレーカーパターンを実装することで、連続エラーを検出するとオープン状態に遷移してAPI呼び出しを即座に停止し、キャッシュ済みの代替応答を返します（要件1）。一定期間後にハーフオープン状態で試験的にAPIを呼び出し、成功すればクローズ状態に自動復帰します（要件2）。サーキットブレーカーの実装は軽量なライブラリを利用でき、他の選択肢と比較してカスタムコードが最小限です（要件3）。B の API Gateway スロットリングはレート制限であり、API障害の検出やフォールバック応答の提供はできません。C の手動停止・再起動はリアルタイムの自動対応ができず、ユーザーへの即時フォールバックの要件を満たしません。D の短いタイムアウト設定ではエラーメッセージがそのまま表示され、ユーザーへの適切なフォールバック応答の要件を満たしません。（スキル2.1.3）",
  "optionExplanations": {
    "A": {
      "correct": true,
      "text": "正解です。Lambda 関数内のサーキットブレーカーパターンは、障害検出後の即時停止・フォールバック応答・API復旧後の自動再開の3要件をすべて満たし、軽量なライブラリの導入により最小のコード変更で実装できます。"
    },
    "B": {
      "correct": false,

...
window.DOMAIN4_QUESTIONS.push({
  ...
  "explanation": "正解はAです。temperature を0.8〜0.9に引き上げるとモデルの出力確率分布がより均一になり、多様で創造的なテキストが生成されやすくなります。top-p を0.9に設定することで確率の高い上位90%のトークンからサンプリングされ、低確率の極端なトークン（意味不明な文章の原因）を排除できます。さらに Bedrock Guardrails のコンテンツフィルターでブランドガイドライン違反を自動検出することで、品質管理も実現できます。Bは temperature 0.1では出力が決定論的になり、多様性が著しく低下します。Comprehend の感情分析は多様性向上には寄与しません。Cは maximum tokens の制限は出力長の制御であり多様性とは無関係です。ショートコピーの結合は文脈の一貫性を損ないます。DはSageMaker でのファインチューニングは投資が大きく、パラメータ調整と Guardrails で解決できる課題に対しては過剰です。（スキル: 4.2.4）",
  "optionExplanations": {
    "A": {
      "correct": true,

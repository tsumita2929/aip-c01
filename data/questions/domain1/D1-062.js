...
window.DOMAIN1_QUESTIONS.push({
  ...
  "optionExplanations": {
    "1": {
      "correct": true,
      "text": "JSON スキーマ検証 → A: FM の出力が期待される構造化データの形式要件（フィールド名、データ型、必須項目の存在）を満たしているかをプログラムで自動検証します。API応答やデータパイプラインに FM の出力を組み込む際に、下流システムでのエラーを防止するために不可欠な品質保証手法です。"
    },
    "2": {
      "correct": true,
      "text": "エッジケーステスト → C: 空入力、極端に長い入力、悪意のある入力（プロンプトインジェクション等）など、通常とは異なる入力に対する FM の堅牢性を検証します。本番環境では予期しない入力が発生するため、プロンプトのリリース前にエッジケースでの振る舞いを確認することが重要です。"
    },
    "3": {
      "correct": true,
      "text": "A/B テスト → B: 異なるプロンプトバリアントを実際のユーザートラフィックに対して配信し、ユーザーフィードバック率やタスク完了率などの指標で効果を統計的に比較します。プロンプト改善の仮説を実データで検証し、定量的な根拠に基づいてプロンプトを最適化できます。"
    }
  },
  "references": [
    {
